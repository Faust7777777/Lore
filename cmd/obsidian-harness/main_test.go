...
func TestRunSmokeP0FullIncludesGovernedNoteIntake(t *testing.T) {
	...
	for runIndex := 0; runIndex < 2; runIndex++ {
		...
		if exitCode != 0 {
			t.Fatalf("run %d expected zero exit code, got %d, stdout = %q, stderr = %q", runIndex+1, exitCode, stdout.String(), stderr.String())
		}
		for _, expected := range []string{
			"P0 smoke passed",
			...
			if !strings.Contains(stdout.String(), expected) {
				t.Fatalf("run %d expected full smoke output to contain %q, got %q", runIndex+1, expected, stdout.String())
			}
		}
	}
}
