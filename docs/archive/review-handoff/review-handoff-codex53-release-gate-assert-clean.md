...
When enabled, it checks after all gate commands succeed:
...
- Confirm the assertion runs before `Pop-Location` while still at repo root, and does not mask an earlier gate failure.
