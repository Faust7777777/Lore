...
`release-gate.ps1` is the targeted governance gate for release candidates. It locks ToolRegistry schema/dispatch tests, MCP boundary/contract tests, daemon watcher and post-scan guardrails, governed note smoke, CLI smoke, the preferred `lore` CLI wrapper, Go SDK v0 contract/transport tests, backend resolve-read-final task-turn coverage, sessionlog task-turn persistence, TUI approval/task-step render tests, and the deterministic visible task-turn acceptance scenario. Use `.\scripts\release-gate.ps1 -Full` to append the full `verify.ps1` suite.
